...
from data_aug.directionalDropStripes import DirectionalSpecAugmentation
from data_aug.frequencyWarp import FrequencyWarpSpecAugmentation
from data_aug.adaptiveDropStripes import AdaptiveSpecAugmentation
from data_aug.specAugmentation import SpecAugmentation
from data_aug.multiScaleDropStripes import MultiScaleSpecAugmentation
from models import Singlebranchspectralclassifier
from pytorch_utils import specmix
from data_aug.jointTimeFrequencyWarp import JointTimeFrequencyWarp
import os
import time
...
def parse_args():
    ...
    parser.add_argument('-c', '--checkpoint_dir',
                        default='E:/bird_sound_classification/BirdMLClassification-main/SSL_Net/models/audio/MEL_resnet18/',
...
def parse_args():
    ...
    parser.add_argument('-c', '--checkpoint_dir',
                        ...
                        help="Directory to save checkpoints (default: checkpoints)")
    parser.add_argument('-s', '--seed', type=int, default=42, help="Random seed for reproducibility")
    parser.add_argument('-a', '--augmentation', nargs='*', choices=['none', 'mixup', 'specaugment', 'RandomReplacementSpecAugmentation', 'specmix'], default=['none'], help="Data augmentation methods to use (default: none)")
    return parser.parse_args()
...
def train(model, train_loader, test_loader, criterion, optimizer, scheduler, device, num_epochs, args):
    print("[INFO] Training MLP...")
    spec_augmenter = SpecAugmentation(time_drop_width=1, time_stripes_num=2, freq_drop_width=4, freq_stripes_num=2) if 'specaugment' in args.augmentation else None
    aug = RandomReplacementSpecAugmentation(time_drop_width=1, time_stripes_num=2, freq_drop_width=4, freq_stripes_num=2, replacement='noise')
    ...
    for epoch in range(num_epochs):
        ...
        print(f'Epoch {epoch + 1}: Train Loss: {train_loss:.4f}, Train Accuracy: {train_accuracy * 100:.2f}%, Test Loss: {test_loss:.4f}, Test Accuracy: {test_accuracy * 100:.4f}%')

    return {'train_loss': train_loss, 'test_loss': test_loss, 'train_accuracy': train_accuracy, 'test_accuracy': test_accuracy}
...
if __name__ == "__main__":
    ...
    optimizer = optim.SGD(model.parameters(), lr=args.learning_rate, momentum=0.9, weight_decay=1e-4)
    scheduler = CosineAnnealingLR(optimizer, T_max=args.train_epochs, eta_min=0, last_epoch=-1)
    final_epoch_info = train(model, train_loader, test_loader, criterion, optimizer, scheduler, args.device, args.train_epochs, args)
    save_model(model, args.checkpoint_dir, final_epoch_info, args)
