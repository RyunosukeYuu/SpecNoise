...
def parse_args():
    ...
    parser.add_argument('-c', '--checkpoint_dir',
                        default='E:/bird_sound_classification/BirdMLClassification-main/SSL_Net/models/audio/STFT_resnet18/',
...
def train(model, train_loader, test_loader, criterion, optimizer, scheduler, device, num_epochs, args):
    ...
    spec_augmenter = SpecAugmentation(time_drop_width=1, time_stripes_num=4,
                                      freq_drop_width=4,
                                      freq_stripes_num=2) if 'specaugment' in args.augmentation else None

    aug = RandomReplacementSpecAugmentation(time_drop_width=1, time_stripes_num=2, freq_drop_width=8,
